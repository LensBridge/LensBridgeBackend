...
public interface PosterRepository extends JpaRepository<Poster, UUID> {
    ...
    @Query("SELECT p FROM Poster p WHERE (p.audience = :aud OR p.audience = com.ibrasoft.minbar.model.board.Audience.BOTH) ORDER BY p.startTime DESC")
    List<Poster> findByAudienceOrBoth(@Param("aud") Audience audience);

    @Query("SELECT p FROM Poster p WHERE (p.audience = :aud OR p.audience = com.ibrasoft.minbar.model.board.Audience.BOTH) AND p.startTime <= :now AND p.endTime > :now ORDER BY p.startTime DESC")
